  ...
  if (isLoading) {
    return (
      <FormField label="ชื่อองค์กร" required>
        <div className="p-3 bg-white/40 rounded-lg border-2 border-orange-200/60">
  ...
  return (
    <FormField label="ชื่อองค์กร" required error={error}>
      {groupCategories.length > 0 ? (
        <div className="flex flex-wrap gap-2">
          {groupCategories.map((category) => (
            <label 
              key={category.id} 
              className={`
                inline-flex items-center px-3 py-2 rounded-full border cursor-pointer
        ...
        <div className="p-4 text-center text-gray-500 bg-gray-50/50 rounded-lg border-2 border-dashed border-gray-200">
          <p className="text-sm">ไม่มีองค์กรให้เลือก</p>
        </div>
      )}

      {/* แสดงคำอธิบายของหมวดหมู่ที่เลือก - Orange Theme */}
      {value && (
        <div className="mt-3 p-3 bg-orange-50/60 border border-orange-200/50 rounded-lg">
          {(() => {
            const selectedCategory = groupCategories.find(cat => cat.id === value);
            return selectedCategory?.description ? (
              <p className="text-sm text-orange-700">
                <span className="font-medium">คำอธิบาย:</span> {selectedCategory.description}
              </p>
            ) : null;
          })()}
        </div>
