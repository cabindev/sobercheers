...
const MonthlySubmissionChart: React.FC = () => {
  ...
  if (!monthlyData.length) {
    return <div className="text-center text-xs text-emerald-600 py-8">ไม่พบข้อมูลรายเดือน</div>;
  }
    ...
    <div className="bg-white h-full flex flex-col">
      ...
      <div className="mt-3 grid grid-cols-2 gap-2">
        <div className="bg-emerald-50 rounded p-2 text-center border border-emerald-200">
          <div className="text-xs text-emerald-700 mb-1">รวมทั้งหมด</div>
          <div className="text-sm font-medium text-emerald-800">{totalSubmissions}</div>
          <div className="text-xs text-emerald-600">องค์กร</div>
        </div>
        
        <div className="bg-emerald-50 rounded p-2 text-center border border-emerald-200">
          <div className="text-xs text-emerald-700 mb-1">เฉลี่ยต่อเดือน</div>
          <div className="text-sm font-medium text-emerald-800">{avgPerMonth}</div>
          <div className="text-xs text-emerald-600">องค์กร</div>
        </div>
        
        <div className="bg-emerald-50 rounded p-2 text-center border border-emerald-200">
          <div className="text-xs text-emerald-700 mb-1">สูงสุด</div>
          <div className="text-sm font-medium text-emerald-800">{maxMonth.count}</div>
          <div className="text-xs text-emerald-600">{maxMonth.month.split(' ')[0]}</div>
        </div>
        
        <div className="bg-emerald-50 rounded p-2 text-center border border-emerald-200">
          <div className="text-xs text-emerald-700 mb-1">ต่ำสุด</div>
          <div className="text-sm font-medium text-emerald-800">{minMonth.count}</div>
          <div className="text-xs text-emerald-600">{minMonth.month.split(' ')[0]}</div>
        </div>
      </div>
    </div>
  );
