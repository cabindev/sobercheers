    ...
    const fetchSoberCheers = async () => {
      try {
        ...
        setFormData({
          ...
          monthlyExpense: data.monthlyExpense?.toLocaleString() || '',
          motivations: Array.isArray(data.motivations) 
            ? data.motivations 
            : typeof data.motivations === 'string' 
              ? JSON.parse(data.motivations) 
              : [],
        });
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (res.status === 200) {
        ...
        throw new Error('Failed to update SoberCheers');
      }
    } catch (error: any) {
